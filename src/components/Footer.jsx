            ...
            <div key={footerLink.key} className='flex flex-col ss:my-0 my-4 min-w-[150px]'>
              ...
              <ul className='list-none mt-4'>
                {footerLink.links.map((Link, index)=>(
                  <li key={Link.index} className={`font-poppins font-normal text-[16px] leading-[24px] text-dimWhite hover:text-[#9dedf0] cursor-pointer ${index!==footerLink.links.length-1 ?'mb-4' : 'mb-0'}`}>
                    {Link.name}
                  </li>
                ))}
              </ul>
